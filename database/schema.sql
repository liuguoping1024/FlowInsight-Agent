...
CREATE TABLE IF NOT EXISTS stock_capital_flow_history (
    ...
    -- 资金流向字段（根据 api-1.md 文档，来自API的f51-f63）
    -- f52: 主力净流入 = 超大单净流入(f56) + 大单净流入(f55)
    main_net_inflow DECIMAL(20, 2) COMMENT '主力净流入额(f52)',
    super_large_net_inflow DECIMAL(20, 2) COMMENT '超大单净流入额(f56)',
    large_net_inflow DECIMAL(20, 2) COMMENT '大单净流入额(f55)',
    medium_net_inflow DECIMAL(20, 2) COMMENT '中单净流入额(f54)',
    small_net_inflow DECIMAL(20, 2) COMMENT '小单净流入额(f53)',
    main_net_inflow_ratio DECIMAL(10, 4) COMMENT '主力净流入占比(f57)',
    small_net_inflow_ratio DECIMAL(10, 4) COMMENT '小单净流入占比(f58)',
    medium_net_inflow_ratio DECIMAL(10, 4) COMMENT '中单净流入占比(f59)',
    large_net_inflow_ratio DECIMAL(10, 4) COMMENT '大单净流入占比(f60)',
    super_large_net_inflow_ratio DECIMAL(10, 4) COMMENT '超大单净流入占比(f61)',
